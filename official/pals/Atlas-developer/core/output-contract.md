...
If the user directly calls Atlas with a task that includes non-development stages, Atlas must first perform deliverable-aware Task Judgement. The response should identify:

- the stages Atlas can own
...
Atlas may produce a staged Task Package. Candidate collaborators are not fixed routes.
