# R108-D Official Pal Index Integration Issue Template
...
| Field | Guidance |
| --- | --- |
| `issue_id` | Stable local id, for example `R108-INT-001`. |
...
| `expected` | What the R108-D gate or source thread required. |
...
| `high` | selected index / README / audit evidence is missing, wrong path was changed, or root entry / `pal.json` loadability is uncertain. |
| `medium` | wording may imply behavior change, Pal Skill / Agent Skill boundary is unclear, prior backfill existence is uncertain, or R108-A/B/C overlap needs repair. |
...
```yaml
issue_id: R108-INT-001
affected_pal: Mira
affected_path: official/pals/Mira-main/skills/index.md
severity: high
finding: Expected Mira skills index is not present, and R108-A validation does not record an equivalent file or deliberate no-write decision.
expected: R108-A either creates the approved skills index, identifies an approved equivalent, or records a no-write decision.
actual: File missing; decision evidence missing.
recommended_fix: Ask R108-A owner for validation evidence or create a scoped R109 repair issue.
